...
RESPONSE=$(docker exec -u 33 master-nextcloud-1 curl -s -u 'admin:admin' \
  ...
  "http://localhost/index.php/apps/softwarecatalog/api/koppelingen-gebruik?_limit=1")
...
RESPONSE=$(docker exec -u 33 master-nextcloud-1 curl -s -u 'admin:admin' \
  -H "Content-Type: application/json" \
  "http://localhost/index.php/apps/softwarecatalog/api/koppelingen-gebruik?_limit=5")
...
if [ -n "$TEST_UUID" ]; then
  ...
  RESPONSE=$(docker exec -u 33 master-nextcloud-1 curl -s -u 'admin:admin' \
    "http://localhost/index.php/apps/softwarecatalog/api/koppelingen-gebruik/${TEST_UUID}")
...
echo "  cd softwarecatalog"
echo "  docker exec -u 33 master-nextcloud-1 bash -c \"cd /var/www/html/apps-extra/softwarecatalog && vendor/bin/phpunit --testsuite 'Integration Tests'\""
