 ...
 * including welcome emails for organizations and users.
 * 
 * @category Service
...
class EmailService
{
     ...
     *
     * @param IMailer         $mailer The Nextcloud mailer service
    ...
    public function sendOrganizationWelcomeEmail(array $organization): bool
    {
        ...
        if (!$organizationEmail) {
            $this->logger->warning('Cannot send welcome email to organization without email address', [
    ...
    public function sendGebruikerWelcomeEmail(array $gebruiker): bool
    {
        ...
        if (!$userEmail) {
            $this->logger->warning('Cannot send welcome email to user without email address', [
                'gebruiker' => $gebruiker
            ]);
            return false;
        }
    ...
    public function sendContactWelcomeEmail(array $contact): bool
    {
        ...
        if (!$contactEmail) {
            $this->logger->warning('Cannot send welcome email to contact without email address', [
                'contact' => $contact
            ]);
            return false;
        }
    ...
    /**
     * Sends an email using Nextcloud's mail service
     *
    ...
    private function sendEmail(
        ...
        try {
            ...
            $senderEmail = $this->config->getSystemValue('mail_from_address', self::DEFAULT_SENDER);
            $senderName = $this->config->getSystemValue('mail_from_name', self::DEFAULT_SENDER_NAME);
            $senderDomain = $this->config->getSystemValue('mail_domain', '');
            
            if ($senderDomain) {
                $senderEmail = $senderEmail . '@' . $senderDomain;
            }
    ...
    public function getSenderEmail(): string
    {
        ...
        if ($senderDomain && !str_contains($senderEmail, '@')) {
            $senderEmail = $senderEmail . '@' . $senderDomain;
        }
    ...
    {
        return $this->config->getSystemValue('mail_from_name', self::DEFAULT_SENDER_NAME);
    }
} 