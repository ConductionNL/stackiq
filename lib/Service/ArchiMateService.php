...
class ArchiMateService
{
    ...
    private function createSectionObject(string $section, string $identifier, array $data, string $modelIdentifier): array
    {
        ...
        else if ($identifier && str_starts_with($identifier, 'id-')) {
  ...
  <relationships>
    ...
    public function getArchiMateStatus(): array
    {
        ...
        try {
            ...
            // Calculate totals
            $totalCount = count($elementObjects) + count($organizationObjects) + count($viewObjects) + count($relationshipObjects) + count($modelObjects) + count($propertyObjects) + count($propertyDefinitionObjects);
    ...
    private function getObjectsWithPagination(string $schemaType, array $query=[]): array
    {
        try {
            ...
            if (!$registerId || !$schemaId) {
                $errorMessage = $isAmefType ? "ArchiMateService: AMEF register or {$schemaType} schema not configured" : "ArchiMateService: Register or {$schemaType} schema not configured";
                $this->logger->error(
                        $errorMessage,
                        [
                            'registerId' => $registerId,
