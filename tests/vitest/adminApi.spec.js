...
import { apiUrl, apiRequest, normaliseFederationStatus } from '../../src/utils/adminApi.js'

describe('adminApi.apiUrl', () => {
	it('builds the app API URL, tolerating leading slashes', () => {
		expect(apiUrl('federation/status')).toBe('/index.php/apps/softwarecatalog/api/federation/status')
		expect(apiUrl('/federation/status')).toBe('/index.php/apps/softwarecatalog/api/federation/status')
	})
})
...
describe('adminApi.apiRequest', () => {
	...
	it('serialises a JSON body and sets the method', async () => {
		...
		await apiRequest('federation/peers', { method: 'POST', body: { peerUrl: 'https://x.org' } }, fetchImpl)
	...
	it('throws the server message on a non-2xx response', async () => {
		...
		await expect(apiRequest('federation/peers', { method: 'POST' }, fetchImpl))
	...
	it('falls back to an HTTP-status error when the body carries no message', async () => {
		...
		await expect(apiRequest('federation/status', {}, fetchImpl)).rejects.toThrow('HTTP 500')
	})
})
...
describe('adminApi.normaliseFederationStatus', () => {
	it('normalises the enriched per-peer shape', () => {
		...
		expect(out.peers[1]).toEqual({ url: 'https://b.org', failures: 4, stale: true, allowed: true })
	})

	it('tolerates a legacy string[] peers payload', () => {
		const out = normaliseFederationStatus({ peers: ['https://legacy.org'] })
		expect(out.peers[0]).toEqual({ url: 'https://legacy.org', failures: 0, stale: false, allowed: true })
