...
import { aggregatePath, normaliseAggregate } from '../../src/utils/reviewAggregate.js'
...
describe('reviewAggregate.aggregatePath', () => {
	it('builds the query path with subjectType and subjectId', () => {
		...
		expect(path).toBe('reviews/aggregate?subjectType=module&subjectId=module-uuid-1')
	})

	it('tolerates missing arguments', () => {
		expect(aggregatePath(undefined, undefined)).toBe('reviews/aggregate?subjectType=&subjectId=')
	})
})

describe('reviewAggregate.normaliseAggregate', () => {
	it('passes through a well-formed response', () => {
		const result = normaliseAggregate({ average: 8.25, count: 4, items: [{ id: '1' }] })
	...
	it('defaults a null average, zero count, and empty items for a malformed/missing response', () => {
		expect(normaliseAggregate(null)).toEqual({ average: null, count: 0, items: [] })
		expect(normaliseAggregate({})).toEqual({ average: null, count: 0, items: [] })
		expect(normaliseAggregate({ average: 'not-a-number', count: 'x', items: 'not-an-array' }))
