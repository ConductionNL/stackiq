...
docker exec -u 33 "$CONTAINER" curl -s -u 'admin:admin' \
  ...
  'http://localhost/index.php/apps/stackiq/api/aangeboden-gebruik/ambtenaar' | jq .
...
docker exec -u 33 "$CONTAINER" curl -s -u 'admin:admin' \
  -H 'Content-Type: application/json' \
  'http://localhost/index.php/apps/stackiq/api/aangeboden-gebruik/ambtenaar?limit=5&offset=0' | jq .
...
docker exec -u 33 "$CONTAINER" curl -s -u 'admin:admin' \
  -H 'Content-Type: application/json' \
  'http://localhost/index.php/apps/stackiq/api/aangeboden-gebruik/afnemer' | jq .
