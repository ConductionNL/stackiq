...
- [~] Confirm engine support for a `scheduled` date-window filter on `eindDatum` before enabling `contract-expiry` — deferred; contract-expiry ships disabled; see Caveats in proposal.md — deferred to downstream cycle (handoff)
- [~] Decide whether supplier delivery requires a relation-traversal recipient resolver or a structured supplier-uid field (file follow-up issue if so) — deferred; see Caveats in proposal.md — deferred to downstream cycle (handoff)
