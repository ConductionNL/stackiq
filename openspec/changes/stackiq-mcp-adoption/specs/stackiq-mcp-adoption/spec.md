# stackiq-mcp-adoption Specification
...
- GIVEN the stackiq register merged with `register.d/stackiq-mcp-adoption.json`
- WHEN OpenRegister's `SchemaDerivedToolProvider` lists MCP tools for the `stackiq` app
- THEN the tool list MUST include `stackiq.module.search` and `stackiq.module.get`
...
- GIVEN `register.d/stackiq-mcp-adoption.json` declares `lead`-style filters such as `module.search.filters = ["naam", "type", "aanbieder", "licentietype", "hostingJurisdictie"]`
...
- THEN no such class MUST exist in the `stackiq` app
...
`lib/Settings/register.d/stackiq-mcp-adoption.json` fragment file;
...
- THEN the diff MUST include `lib/Settings/register.d/stackiq-mcp-adoption.json`
...
- [ ] `register.d/stackiq-mcp-adoption.json` declares `configuration.x-openregister-mcp` on exactly the 9 curated schemas listed in REQ-001.
