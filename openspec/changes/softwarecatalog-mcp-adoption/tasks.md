# Tasks: softwarecatalog-mcp-adoption

## Implementation Tasks

### Task 1: Add the `register.d/softwarecatalog-mcp-adoption.json` fragment
- **spec_ref**: `openspec/changes/softwarecatalog-mcp-adoption/specs/softwarecatalog-mcp-adoption/spec.md#req-001-curated-read-only-mcp-dialect-on-the-9-core-catalogue-schemas`
- **files**: `lib/Settings/register.d/softwarecatalog-mcp-adoption.json`
- **acceptance_criteria**:
  - GIVEN the exact JSON in design.md WHEN it is written to `lib/Settings/register.d/softwarecatalog-mcp-adoption.json` THEN all 9 curated schemas (`module`, `moduleVersie`, `dienst`, `organisatie`, `contactpersoon`, `koppeling`, `compliancy`, `gebruik`, `contract`) carry a `configuration.x-openregister-mcp` block with `enabled: true`
...
- **spec_ref**: `openspec/changes/softwarecatalog-mcp-adoption/specs/softwarecatalog-mcp-adoption/spec.md#req-002-every-declared-search-filter-names-a-real-schema-property`
- **files**: `lib/Settings/register.d/softwarecatalog-mcp-adoption.json`, `lib/Settings/softwarecatalogus_register.json` (read-only reference)
- **acceptance_criteria**:
...
- **spec_ref**: `openspec/changes/softwarecatalog-mcp-adoption/specs/softwarecatalog-mcp-adoption/spec.md#req-004-mcp-dialect-is-declared-via-a-register-fragment-not-the-monolith`
- **files**: `lib/Settings/register.d/softwarecatalog-mcp-adoption.json`
- **acceptance_criteria**:
  - GIVEN the new fragment WHEN run through `python3 -m json.tool` THEN it parses with zero errors
  - GIVEN a dev Nextcloud instance WHEN `SettingsService::loadSettings()` re-runs (fragment signature changed) THEN the merged register imports without error and `softwarecatalog.module.search`/`.get` (and the other 8 schemas' tools) appear in OpenRegister's MCP tool listing
- [ ] Implement
- [ ] Test

### Task 4: Add a CHANGELOG entry
- **spec_ref**: `openspec/changes/softwarecatalog-mcp-adoption/specs/softwarecatalog-mcp-adoption/spec.md#req-003-mcp-tools-are-derived-without-app-level-php`
- **files**: `CHANGELOG.md`
- **acceptance_criteria**:
