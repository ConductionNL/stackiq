...
**OpenSpec changes**:
- [suite-wizard](../../changes/suite-wizard/)

## Purpose
Lets a catalogue editor register an application **suite** (e.g. a bundled municipal "leefomgeving" product) and attach its existing member applications in one guided pass, replacing the retired incumbent "product" concept per VNG Softwarecatalogus issue #242 and softwarecatalog#372. Provides a suite index + detail page and surfaces suite membership from a module's own detail page, over the existing `suite` OpenRegister schema, per ADR-001/ADR-008/ADR-012.
