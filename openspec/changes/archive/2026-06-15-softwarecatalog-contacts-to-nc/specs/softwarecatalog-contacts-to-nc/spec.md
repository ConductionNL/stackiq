# softwarecatalog-contacts-to-nc

**Status:** proposed
**Scope:** softwarecatalog
**Tier:** leaf
**Depends on:** Nextcloud addressbook (`OCP\Contacts\IManager`), cross-app
interface contract #2 (people/organisations/contacts → NC addressbook keyed by
`contactsUid`); reuses `ContactSyncService` pattern (ADR-019, ADR-022, ADR-012).
