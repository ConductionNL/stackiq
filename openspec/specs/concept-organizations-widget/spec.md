...
## Purpose
Registers the Concept Organisations dashboard widget, loading its frontend assets (runtime, vendor, nc-vue, and widget chunks plus stylesheet) in correct dependency order through the Nextcloud asset framework under the stackiq application namespace.
...
`ConceptOrganisatiesWidget::load()` MUST register, in this order: (1) the webpack runtime chunk `stackiq-runtime`, (2) the shared vendor chunk `stackiq-shared-vendor`, (3) the shared nc-vue chunk `stackiq-shared-nc-vue`, (4) the widget entry chunk `stackiq-conceptOrganisatiesWidget`. It MUST also register the `dashboardWidgets` stylesheet. All registrations MUST go through `OCP\Util::addScript` / `OCP\Util::addStyle` using `Application::APP_ID` as the application namespace.
...
- GIVEN `Application::APP_ID = "stackiq"`
- WHEN `load()` executes
- THEN every `Util::addScript` / `Util::addStyle` call MUST use `"stackiq"` as the `application` argument
