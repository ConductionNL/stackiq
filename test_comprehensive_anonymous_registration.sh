...
echo "🧪 Comprehensive Anonymous User Registration Test"
echo "=================================================="
echo ""
...
# Test data
TEST_DATA='{
    "naam": "Comprehensive Test Organization",
    "website": "https://comprehensive-test.org",
    "type": "Gemeente",
    "beoordeling": "actief",
    "contactpersonen": [
        {
        ...
        },
        {
            "voornaam": "Secondary",
            "achternaam": "Contact",
            "email": "secondary.contact@test.org",
            "telefoon": "+31 555 555 559",
            "functie": "Developer"
        }
    ]
}'
...
echo "1. Anonymous user submits organization registration via OpenConnector"
echo "2. System creates organization object with UUID (e.g., 71658220-8409-4139-b383-b521e637a493)"
echo "3. System creates organization entity with SAME UUID"
echo "4. System creates user accounts for contact persons"
echo "5. System adds users to organization entity"
echo "6. System sets ownership on objects to newly created users"
echo "7. System sets organization references on all objects"
echo ""

echo "🚀 Step 1: Sending Anonymous Registration Request"
echo "-------------------------------------------------"
response=$(curl -s -w "%{http_code}" \
    -X POST "$BASE_URL/index.php/apps/openconnector/api/endpoint/register" \
...
if [ "$http_code" -eq 200 ] || [ "$http_code" -eq 201 ]; then
    ...
    if [ -n "$uuid" ] && [ "$uuid" != "null" ]; then
        ...
        echo "⏳ Step 2: Waiting for Processing (5 seconds)"
        echo "---------------------------------------------"
        sleep 5
    ...
    echo "❌ Step 1 FAILED: Registration failed with HTTP $http_code"
    echo ""
    echo "Troubleshooting:"
    echo "1. Check if OpenConnector app is enabled"
    echo "2. Check if Docker environment is running"
    echo "3. Check if API endpoints are accessible"
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    docker-compose exec -u 33 nextcloud php /var/www/html/occ user:delete primary.contact@test.org
    docker-compose exec -u 33 nextcloud php /var/www/html/occ user:delete secondary.contact@test.org
